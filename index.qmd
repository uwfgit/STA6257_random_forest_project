...
### Potential additions to methods/modeling
#### Decision trees
Decision trees start with a basic question, such as, “Should I surf?” From there, you can ask a series of questions to determine an answer. “Is it a long period swell?” or “Is the wind blowing offshore?”.
These questions make up the decision nodes in the tree, acting as a means to split the data. Each question aids in the arrival of a final decision, which would be denoted by the leaf node.
Observations that fit the criteria will follow the “Yes” branch, and those that do not will follow the alternate path.
...
##### Ensemble Method
Ensemble learning methods are made up of a set of classifiers (i.e., decision trees), and their predictions are aggregated to identify the most popular result.

- Bagging
  - Also known as bootstrap aggregation
  - A random sample of data in a training set is selected with replacement, meaning the individual data points can be chosen more than once.
...
##### Assumptions
- Some of the actual values in the feature variable of the dataset should be present so the classifier can predict accurate results.
- The predictions from each tree must have very low correlations.

#### Gini Impurity
- The Gini coefficient is a measure of inequality between 0 and 1, where a value closer to 1 indicates more inequality.  This is used to calculate inequality in income and wealth, but can be used in other problems where a measure of inequality is needed.
...
- There are subsets of each observation which is the B-bootstrap and each bagged tree makes use of around two thirds of population data.The remaining one-third population data that isn’t used to fit the given bagged tree are the Out of bag observations.

- Overfitting can be a challenge for Decision trees.This can be solved by pre-pruning and post-pruning. Pre-pruning prevents the tree from growing too big  and post pruning allows the tree to grow to its full capacity and gets rid of branches based on different criteria.Using the averages of multiple models would be a Random Forest model.
...
### TODO: header of the description of the graph
```{r}
ggplot(data = data, aes(x = Status, y = Range, fill = Status)) +
  geom_boxplot() +
  labs(title = "Distribution of Range by Conservation Status") +
  theme_bw() +
  ylim(0,100000)
```
### TODO: header of the description of the graph
```{r}
table1 <- table(data$Habitat_loss, data$Biomes,
                dnn = c("Habitat Loss", "Biomes"))
addmargins(table1)

barplot(table1,
    	ylab="Count of Plant Species", xlab="Biomes",
    	main="Distribution of Habitat Loss by Geographical Regions",
    	col=c("turquoise4", "turquoise2"),
    	beside <- FALSE, xlim=c(0,0.8), width=.05)
legend("right", title="Habitat Loss",
   	legend= sort(unique(data$Habitat_loss)),
   	fill =c("turquoise4", "turquoise2"), box.lty=0)
```
### TODO: header of the description of the graph
```{r}
Group2 <- factor(data$Group, levels=c("NotThr","Thr","EX"))
plt3 <- ggplot(data = data,
               aes(x=Biomes, y = Group2)) +
  geom_bar(aes(y= Group,fill=Group2),
           stat="identity",
           alpha=.7) +
  scale_fill_brewer(palette=3) +
  theme_bw() +
  labs(title = "Distribution of Plant Species by Status", 
       y = "Conservation Group")
plt3
```
### TODO: header of the description of the graph
```{r}
...
library(corrplot)
library(DescTools)
CatData <- data %>% select(!(Range))
corrplot::corrplot(DescTools::PairApply(CatData,DescTools::CramerV))
```
